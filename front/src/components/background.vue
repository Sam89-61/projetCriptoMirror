<template>
    <div>
        <!-- Canvas fixé derrière tout le contenu -->
        <div class="fixed inset-0 z-0 bg-black overflow-hidden pointer-events-none">
            ...
            <div v-if="outerVignette"
                class="top-0 left-0 absolute bg-[radial-gradient(circle,rgba(0,0,0,0)_60%,rgba(0,0,0,1)_100%)] w-full h-full" />
            <div v-if="centerVignette"
                class="top-0 left-0 absolute bg-[radial-gradient(circle,rgba(0,0,0,0.8)_0%,rgba(0,0,0,0)_60%)] w-full h-full" />
        </div>
        <!-- Contenu par-dessus -->
...
const initializeLetters = (columns: number, rows: number) => {
    ...
    letters.value = Array.from({ length: totalLetters }, () => ({
        ...
        targetColor: getRandomColor() || '#fff',
        colorProgress: 1
    }));
};
...
const updateLetters = () => {
    ...
    for (let i = 0; i < updateCount; i++) {
        ...
        letters.value[index].targetColor = getRandomColor() || '#fff';
